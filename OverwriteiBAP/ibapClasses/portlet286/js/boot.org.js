...
if (!window.zk) { // avoid eval twice
	...
	// Customization
	/**
	 * Creates the message box to notify the user that ZK client engine is
	 * booting. Developer can override this method to provide a customized
	 * message box.
	 * 
	 * @param id
	 *            the progress box's ID
	 * @param msg
	 *            the message
	 * @param x
	 ...
	 * @param mask
	 *            sets whether show the modal_mark (since 3.0.2)
	 * @param center
	 *            sets whether center the loading bar (since 3.0.2)
	 */
	if (!window.Boot_progressbox) { // not customized
		Boot_progressbox = function(id, msg, x, y, mask, center) {
			...
			if (mask) {
				...
				html += '><div id="zk_mask" class="z-modal-mask" style="display:block;left:'
						+ ix
						+ 'px;top:'
						+ iy
						+ 'px;" z.x="'
			...
			html += ' id="zk_loading" class="z-loading" style="left:' + x
					+ ';top:' + y + ';visibility: hidden;"' + ' z.x="' + ix
					+ '" z.y="' + iy + '"><div class="z-loading-indicator">'
					+ '<img class="z-loading-icon" alt="..." src="'
					+ zk.getUpdateURI('/web/img/spacer.gif') + '"/> ' + msg
					+ '</div></div></div>';
			...
			if (center) {
				if (el) {
					el.style.left = (zk.innerWidth() - el.offsetWidth) / 2 + ix
							+ "px";
					el.style.top = (zk.innerHeight() - el.offsetHeight) / 2
							+ iy + "px";
	 ...
	 * processing. Developer can override this method to provide a customized
	 * message box.
	 * 
	 * @param id
	 *            the progress box's ID
	 ...
	 *            the message
	 * @param mask
	 *            sets whether show the modal_mark
	 * @since 3.0.6
	 */
	 ...
	 * 
	 * <p>
	 * Note: If you want to stop the propagation of the event, you can return
	 * false at that listener function
	 * </p>
	 * 
	 * For example,
	 * 
	 ...
	 * 
	 * @param {Object}
	 *            cmp an ID or a zk component
	 * @param {String}
	 *            evtnm The name of event to listen for
	 * @param {Function}
	 *            fn handler The method the event invokes
	 * @since 3.5.0
	 */
	 ...
	 *            evtnm The name of event to listen for
	 * @param {Function}
	 *            fn The handler to remove
	 * @since 3.5.0
	 */
	zk.un = function(cmp, evtnm, fn) {
		var ls = zk.find(cmp), fns = ls[evtnm];
		if (fns) {
			fns.remove(fn);
			if (!fns.length)
				delete ls[evtnm];
	 ...
	 *            cmp an ID or a zk component
	 * @param {String}
	 *            evtnm The name of event to listen for, if any. Otherwise, `*
	 ...
	 * @param {Object}
	 *            scope The scope in which to execute the listener function. The
	 *            listener function's "this" context.
	 * @since 3.5.0
	 */
	...
	/**
	 * Listen a browser event. Why not to use prototype's Event.observe?
	 * Performance.
	 */
	...
	if (zk.ie) { // Bug 1741959: avoid memory leaks
		...
		/**
		 * Unlisten events associated with the specified ID. Bug 1741959: IE
		 * meory leaks
		 */
	...
	zk.disableESC = function() {
		if (!zk._noESC) {
			...
			window.onerror = function(msg, url, lineno) {
				...
				// It is annoying to show error if google analytics's js not
				// found
				var au = zkau.uri();
	 ...
	 */
	function $es(id) {
		return typeof id == 'string' ? id ? document.getElementsByName(id)
				: null : id;
	}
	/**
	...
	/**
	 * Note: it is easy to cause problem with EMBED, if we use prototype's z$()
	 * since it tried to extend the element.
	 * 
	 * @param alias
	 *            a name is postfixed with the id via the "!" word. (since
	 *            3.5.0) for example, if alias is "real", the "id!real" element
	 *            is returned, if any.
	 */
	function $e(id, alias) {
		if (id && id.id)
	...
	/**
	 * A control might be enclosed by other tag while event is sent from the
	 * control directly, so...
	 */
	function $uuid(n) {
		if (typeof n != 'string') {
	...
	/**
	 * Returns the id of the first element which can be found from its parent
	 * node, inclusive itself. Otherwise, "" is assumed.
	 * 
	 * @param {Object}
	 *            n an element.
	 * @since 3.0.6
	...
	/**
	 * Returns the real element (ends with !real). If a component's attributes
	 * are located in the inner tag, i.e., you have to surround it with span or
	 * other tag, you have to place uuid!real on the inner tag
	 * 
	...
	/**
	 * Returns the enclosing element (not ends with !real). If not found, cmp is
	 * returned.
	 */
	function $outer(cmp) {
		var id = $uuid(cmp);
	 ...
	 */
	/*
	 * function $peer(id) { return id ? $e( id.endsWith("!real") ?
	 * id.substring(0, id.length-5): id+"!real"): null; }
	 */
	/**
	...
	/**
	 * Returns the parent node of the specified element. It handles virtual
	 * parent.
	 */
	function $parent(n) {
		var p = zk._vpts[n.id];
	...
	/**
	 * Sets virtual parent. It is used if a popup is limited (cropped) by a
	 * parent div.
	 * 
	 * @since 3.0.0
	 */
	...
	/**
	 * unsetVParent if it is a child of the specified node, n. Note: including
	 * itself.
	 * 
	 * @param hide
	 *            to avoid some annoying effect you can specify whether to hide
	 *            before unsetting the parent. Note: since it is hidden,
	 *            x.style.display will be changed -- use this option with a lot
	 *            of cares
	 * @since 3.0.0
	 */
	...
	// Note: we have to use string to access {}. Otherwise, the behavior is
	// strange
	zk._vpts = {}; // a map of virtual parent (n.id, n's parent)

	/**
	 * Returns the nearest parent element, including el itself, with the
	 * specified type.
	 */
	...
	;
	/**
	 * Returns whether an element is visible. Returns false if none-existence.
	 * Returns true if no style.
	 * 
	 * @param {Boolean}
	 *            strict if true, it also checks the visibility property.(since
	 *            3.0.4)
	 */
	function $visible(el, strict) {
		return el
				&& (!el.style || (el.style.display != "none" && (!strict || el.style.visibility != "hidden")));
	}
	...
	function getZKAttr(el, nm) {
		...
		try {
			...
			return null; // IE6: failed if el is TABLE and attribute not
							// there
		}
	}
	/**
	 ...
	 * initialized.
	 * <p>
	 * Note: it is called after all components are initialized (init, beforeSize
	 * and onSize), so it might be better to be called as addAfterInit
	 * <p>
	 * The function is removed from the list right before invoked, so it won't
	 * be called twice (unless you call zk.addInit again).
	 * 
	 * @param front
	 *            whether to add the function to the front of the list
	 * @param {String}
	 *            unique whether not to add if redundant. If any, fn is added
	 *            only if fn was not added before.
	 */
	zk.addInit = function(fn, front, unique) {
		if (typeof unique == "string") {
			if (zk._initids[unique])
				return;
	...
	/**
	 * Adds a function that will be invoked 25 milliseconds, after all
	 ...
	 * However, the function added by addInitLater is invoked with a timer that
	 * is called 25 milliseconds. Thus, it is designed to add functions that
	 * cannot be called immediately after initialization (zkType.init).
	 * 
	 * <p>
	 * The function is removed from the list right before invoked, so it won't
	 * be called twice (unless you call zk.addInitLater again).
	 * 
	 * @param front
	 *            whether to add the function to the front of the list
	 * @param {String}
	...
	/**
	 * Adds a function for module initialization. It is called after all
	 * javascript fies are loaded, and before initializing the components.
	 * 
	 * <p>
	 * In other words, ZK invokes functions added by zk.addBeforeInit, then
	 * initializes all components, and finally invokes functions added by
	 * zk.addInit.
	 * 
	 * <p>
	 * The function is removed from the list right before invoked, so it won't
	 * be called twice (unless you call zk.addBeforeInit again).
	 */
	zk.addBeforeInit = zk.addModuleInit = function(fn) { // addModuleInit is
															// for backward
															// compatible to
															// 3.0.3 and earlier
		zk._bfinits.push(fn);
	...
	/**
	 * Adds a component that must be initialized after all modules are loaded
	 * and initialized.
	 */
	...
	/**
	 * Adds a function that will be invoked after all components are cleaned up.
	 * Unlike zk.addCleanupLater, the components being cleaned up are still
	 * available when fn is called.
	 * <p>
	 * Note: it is called after all components are initialized.
	 * <p>
	 * The function is removed from the list right before invoked, so it won't
	 * be called twice (unless you call zk.addInit again).
	 * 
	...
	/**
	 * Adds a function that will be invoked 25 milliseconds, after all
	 * components are cleaned up. Note: when fn is called, the component being
	 * cleaned up may be removed.
	 * 
	 * <p>
	 * The function is removed from the list right before invoked, so it won't
	 * be called twice (unless you call zk.addInitLater again).
	...
	/**
	 * Adds a function that will be invoked before the browser is unloading the
	 * page. If the function returns a string, then the whole execution will
	 * stop and the string is returned to browser (to warning the user). If
	 * nothing is returned, the following functions will be invoked.
	 * 
	 * @param front
	 *            whether to add the function to the front of the list
	 * @since 3.0.0
	...
	/**
	 * Called when window.onbeforeunload is called. Note: you rarely need to
	 * invoke this directly (except au.js).
	 * 
	 * @since 3.0.0
	 */
	...
	/**
	 * Unwatches a list of ZK callbacks. Example, zk.unwatch(n, "onVisi",
	 * "onSize"), and then zkType.onVisi and zkType.onSize will be ignored for
	 * the specified component Note: it cannot be called in zkType.init().
	 * 
	 * @since 3.0.5
	 */
	...
	/**
	 * Watches a list of ZK callbacks. Example, zk.watch(n, "onVisi", "onSize"),
	 * and then zkType.onVisi and zkType.onSize will be called back for the
	 * specified component. By default, they will be called back, so you don't
	 * need to invoke this method unless you call zk.unwatch() to stop callback.
	...
	zk.watch = function(n) {
		...
		for ( var as = arguments, j = as.length; --j > 0;)
			switch (as[j]) {
			case "onVisi":
				zk._watch(n, zk._visicmps);
				break;
			case "onHide":
				zk._watch(n, zk._hidecmps);
				break;
			case "onSize":
				zk._watch(n, zk._szcmps);
				break;
			case "beforeSize":
				zk._watch(n, zk._bfszcmps);
				break;
			case "onScroll":
				zk._watch(n, zk._scrlcmps);
			}
	};
	...
	/**
	 * Invokes the specified function that depends on the specified module. If
	 * the module is not loaded yet, it will be loaded first. If it is loaded,
	...
	/**
	 * Registers a script that will be evaluated when the specified module is
	 * loaded.
	 * 
	 * @param script
	 *            a piece of JavaScript, or a function
	 * @since 3.0.6
	 */
	 ...
	 * 
	 * <p>
	 * To load a JS file that other modules don't depend on, use zk.loadJS.
	 * 
	 * @param nm
	 *            the module name if no / is specified, or filename if / is
	 *            specified, or URL if :// is specified.
	 * @param initfn
	 *            the function that will be added to zk.addBeforeInit
	 * @param ckfn
	 *            used if URL (i.e., xxx://) is used as nm and the file being
	 *            loaded doesn't invoke zk.ald(), or we have to check more
	 *            condition than calling back zk.ald().
	 * @param modver
	 *            the version of the module, or null to use zk.getBuild(nm)
	 */
	...
	/**
	 * Loads the required module for the specified component. Note: it DOES NOT
	 * check any of its children.
	 * 
	...
	zk.loadByType = function(n) {
		...
		if (type) {
			var j = type.lastIndexOf('.');
			if (j > 0)
				zk._loadByType(type.substring(0, j), n);
			return true;
		}
		return false;
	};
	...
	zk._load = function(nm, modver, dtid, ckfn) {
		...
		// See also
		// Bug 1815074: IE bug: zk.ald might be called before appendChild
		// returns

		zk._bld();
		var e = document.createElement("script"), prefix = "/web", uri = nm;
		e.type = "text/javascript";

		if (ckfn)
			zk._ckfns.push(ckfn);
		else
			prefix += "/_zcbzk.ald-" + zk._jscnt++;
		...
		if (uri.indexOf("://") > 0) {
			...
			uri = j >= 0 ? uri.substring(0, j) + ".js." + uri.substring(j + 1)
					: uri + ".js";
			if (uri.charAt(0) != '/')
				uri = '/' + uri;
			e.charset = "UTF-8";
			if (!modver)
				modver = zk.getBuild(nm);
	...
	zk.ald = function(jscnt) {
		if (zk._jsmap[jscnt])
			return; // invoked twice (possible since timeplot or other might use
					// the same prefix _zcb... to load their modules)
		...
		if (--zk.loading) {
			...
			// reset the status of loading JS file first, otherwise, it will
			// cause zk.load() out of sync.
			zk._jsmap = {};
			zk._jscnt = 0;
			...
			if (zk._ready)
				zk._evalInit(); // zk._loadAndInit might not finish
		}
	};
	/**
	 ...
	 * 
	 * @param {String}
	 *            id an element Id, such as "zk_loadprog" or "zk_prog"
	 * @since 3.5.0
	 */
	zk.cleanAllMask = function(id) {
		...
		if (n)
			zk.remove(n);
		for ( var c = zk._ctpgs.length, n = $e(zk._ctpgs[--c] + "!progbox"); n; n = $e(zk._ctpgs[--c]
				+ "!progbox"))
			zk.remove(n);
	};
	zk._updCnt = function() {
	...
	zk._loadAndInit = function(inf) {
		...
		for ( var j = 0; inf.stk.length;) {
			...
			if (n.nodeType == 1) {
				try {
					if (!zk.ie) {
						// FF remembers the previous value that user entered
						// when reload
						// We have to reset them because the server doesn't know
						// any of them
						switch ($tag(n)) {
						case "INPUT":
							if (n.type == "checkbox" || n.type == "radio") {
								...
								if (zk.opera)
									zk.setOuterHTML(n, zk.getOuterHTML(n));
								break;
							}
				...
				} // IE7 failed if href contains incorrect encoding
				...
				// Test if a (non-owned) page included by other content (e.g.
				// JSP)
			...
			// Since 3.0.2, we introduce z.skipdsc to stop parsing the
			// descendants.
	...
	/**
	 * Fix bug 1635685 and 1612312 (IE/IE7 only): IE invokes onbeforeunload if
	 * <a href="javascript;"> is clicked (sometimes) It actually ignores
	 * window.beforeunload temporary by set zk.skipBfUnload.
	 */
	if (zk.ie) {
	...
	zk._evalInit = function() {
		do {
			...
			// Note: if loading, zk._doLoad will execute zk._evalInit after
			// finish
			for ( var j = 0; zk._initcmps.length && !zk.loading;) {
				...
				if (type) {
					...
					if (o) {
						...
						if (o["onVisi"])
							zk._tvisicmps.push(n.id); // child-first
						if (o["onHide"])
							zk._thidecmps.push(n.id); // child-first
						if (o["onSize"])
							zk._tszcmps.push(n.id); // child-first
						if (o["beforeSize"])
							zk._tbfszcmps.push(n.id); // child-first
						if (o["onScroll"])
							zk._tscrlcmps.push(n.id); // child-first
					}
				}
			...
			if (!zk.loading) {
				...
				// since beforeSize/onSize might zk.unwatch, add to _szcmps
				// first
				...
				for ( var es = zk._tbfszcmps; es.length;) {
					var n = $e(es.pop()); // parent at the end, so pop the end
											// first
					if ($visible(n))
						zk.eval(n, "beforeSize");
				}
			...
			if (!zk.loading && !zk._initfns.length) {
				...
				&& (zk._bfinits.length || zk._initcmps.length || zk._initfns.length));
	 ...
	 * 
	 * It assumes fn is a method name of a object called "zk" + type (in
	 * window). Nothing happens if no such object or no such method.
	 * 
	 * @param n
	 *            the component
	 * @param fn
	 *            the method name, e.g., "init"
	 * @param type
	 *            the component type. If omitted, $type(n) is assumed.
	 * @return the result
	 */
		...
		if (type) {
			...
			if (o) {
				...
				if (f) {
					try {
						var args = [ n ];
						for ( var j = arguments.length - 2; --j > 0;)
							// 3->1, 4->2...
	...
	zk._cleanupAt = function(n) {
		...
		if (type) {
			...
			zkau.cleanupMeta(n); // note: it is called only if type is
									// defined
	 ...
	 * 
	 * @param n
	 *            the topmost element that has become visible. Note: n has
	 *            become visible when this method is called. If null, all
	 *            elements will be handled
	 */
	zk.onVisiAt = function(n) {
		// Note: process from last since zk.unwatch assumes it
		for ( var elms = zk._visicmps, j = elms.length; --j >= 0;) { // parent
																		// first
			var elm = $e(elms[j]);
	 ...
	 * 
	 * @param n
	 *            the topmost element that will become invisible. Note: n is
	 *            still visible when this method is called. If null, all
	 *            elements will be handled.
	 */
	zk.onHideAt = function(n) {
		// Bug 1526542: we have to blur if we want to hide a focused control in
		// gecko and IE
		...
		// Note: process from last since zk.unwatch assumes it
		for ( var elms = zk._hidecmps, j = elms.length; --j >= 0;) { // parent
																		// first
			var elm = $e(elms[j]);
			for ( var e = elm; e; e = $parent(e)) {
	...
	/**
	 * To notify a component that its parent's size is changed. All descendants
	 * of n is invoked if onSize is declared. The invocation of onSize is
	 * parent-first (and then child).
	 * 
	 * @param n
	 *            the topmost element whose size is changed. If null, the
	 *            browser's size is changed and all elements will be handled.
	 * @since 3.0.4
	 */
	zk.onSizeAt = function(n) {
		// Note: process from last since zk.unwatch assumes it
		for ( var elms = zk._szcmps, j = elms.length; --j >= 0;) { // parent
																	// first
			var elm = $e(elms[j]);
			for ( var e = elm; e; e = $parent(e)) {
				if (!$visible(e))
	...
	/**
	 * To notify a component that its parent's size WILL be changed. All
	 * descendants of n is invoked if beforeSize is declared. The invocation of
	 * beforeSize is parent-first (and then child).
	 * 
	 * <p>
	 * The typical use is: call beforeSizeAt first, adjust the width/height and
	 * then call onSizeAt.
	 * 
	 * <p>
	 * The component usually cleans up in zkXxx.beforeSize, if necessary, and
	...
	zk.beforeSizeAt = function(n) {
		// Note: process from last since zk.unwatch assumes it
		for ( var elms = zk._bfszcmps, j = elms.length; --j >= 0;) { // parent
																		// first
			var elm = $e(elms[j]);
			for ( var e = elm; e; e = $parent(e)) {
				if (!$visible(e))
	...
	/**
	 * To notify a component that the parent is scrolled. All descendants of n
	 * is invoked if onScroll is declared. The invocation of onScroll is
	 * parent-first (and then child).
	 * 
	 * @param n
	 *            the topmost element which is scrolled. If null, the browser's
	 *            size is changed and all elements will be handled.
	 * @since 3.0.5
	 */
	zk.onScrollAt = function(n) {
		if (zkau.valid)
			zkau.valid.onScrollAt(n); // Bug #1819264
		// Note: process from last since zk.unwatch assumes it
		for ( var elms = zk._scrlcmps, j = elms.length; --j >= 0;) { // parent
																		// first
			var elm = $e(elms[j]);
			for ( var e = elm; e; e = $parent(e)) {
				if (!$visible(e))
	 ...
	 * 
	 * @param uri
	 *            Example, "/a/b.css". It will be prefixed with zkau.uri() +
	 *            "/web", unless http:// or https:// is specified
	 */
	 ...
	 * 
	 * @param uri
	 *            the URI. Note: it is assigned to the href attribute directly
	 *            without prefix with /web
	 * @param id
	 *            the identifier (the id attribute). Optional.
	 * @since 3.0.2
	 */
	 ...
	 *            "/web", unless http:// or https:// is specified
	 * @param fn
	 *            the function to execute after loading. It is optional. Not
	 *            function under safari
	 */
	 ...
	 */
	zk._getUpdateURI = function (au, uri, ignoreSessId, modver, dtid) {
	...
	/**
	 * Returns the proper URI taking the Portlet 2.0 resource URL into account.
	 *  This URI is directed at the originating portlet. The p_p_resource_id parameter
	 *  contains the zk resource URI.
	 * 
	 * @param ignoreSessId
	 *            whether not to append session ID.
	 * @param modver
	 *            the module version to insert into uri, or null to use
	 *            zk.build. Note: modver is used if uri starts with /uri
	 */
	zk.getUpdateURI = function(uri, ignoreSessId, modver, dtid) {
		...
		if (!reg.test(au))
			...
			return zk._getUpdateURI(au,uri,ignoreSessId,modver,dtid);

		// the modified zk URI is passed through the resource id parameter after being escaped
		return au.replace(/&p_p_resource_id=([^&$]+)/, "&p_p_resource_id="
				+ escape(zk._getUpdateURI(RegExp.$1,uri,ignoreSessId,modver,dtid)));
	};
	...
	zk.progress = function(timeout) {
		zk.progressing = true;
		if (timeout > 0)
			setTimeout(zk._progress, timeout);
		else
			zk._progress();
	};
	...
	zk._domsg = function() {
		if (zk._msg) {
			...
			if (!console) {
				...
				console = $e("zk_msg");
				var d = $e("zk_debugbox");
				d.style.top = zk.innerY() + zk.innerHeight() - d.offsetHeight
						- 20 + "px";
				d.style.left = zk.innerX() + zk.innerWidth() - d.offsetWidth
						- 20 + "px";
	...
	zk.error = function(msg) {
		...
		var html = '<div style="position:absolute;z-index:99000;padding:3px;left:'
				+ (zk.innerX() + 50)
				+ 'px;top:'
				+ (zk.innerY() + 20)
				+ 'px;width:550px;border:1px solid #963;background-color:#fc9" id="'
				+ id
				+ '"><table cellpadding="2" cellspacing="2" width="100%"><tr valign="top">'
				+ '<td width="20pt"><button onclick="zkau.sendRedraw()">redraw</button>'
				+ '<button onclick="zk._msgclose(this)">close</button></td>'
				+ '<td style="border:1px inset">' + zk.encodeXML(msg, true) // Bug
																			// 1463668:
																			// security
				+ '</td></tr></table></div>';
	...
	zk._ckfns = []; // functions called to check whether a module is loaded
					// (zk._load)
	zk._visicmps = []; // an array of component's ID that requires
						// zkType.onVisi; the child is in front of the parent
	zk._hidecmps = []; // an array of component's ID that requires
						// zkType.onHide; the child is in front of the parent
	zk._szcmps = []; // an array of component's ID that requires
						// zkType.onSize; the child is in front of the parent
	zk._bfszcmps = []; // an array of component's ID that requires
						// zkType.beforeSize; the child is in front of the
						// parent
	zk._scrlcmps = []; // an array of component's ID that requires
						// zkType.onScroll; the child is in front of the parent
	...
	if (zk.ie && !zk.https()) {
		...
		document
				.write('<script id="_zie_load" defer src="javascript:void(0)"><\/script>');
		var e = $e("_zie_load");
		...
		// Bug 1619959: FF not always fire DOMContentLoaded (such as in 2nd
		// iframe),
